...
public interface CarritoRepository extends JpaRepository<Carrito,Long> {
    ...
    Optional<Carrito> findByUsuarioAndActivo(Long usuarioId, Boolean activo);

    @Query("SELECT c FROM Carrito c WHERE c.usuario.id = :usuarioId AND c.activo = :activo ORDER BY c.fechaActualizacion DESC")
    Optional<Carrito> findCarritoActivoByUsuario(@Param("usuarioId") Long usuarioId, @Param("activo") Boolean activo);
